...
class ReconocimientoFacialPage extends StatefulWidget {
  const ReconocimientoFacialPage({super.key});
...
class _ReconocimientoFacialPageState extends State<ReconocimientoFacialPage> {
  ...
  Future<void> _captureAndSend() async {
    ...
    try {
      ...
      // TODO: enviar `picture.path` al backend (POST reconocimiento facial)
      await Future.delayed(const Duration(seconds: 2));

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Foto enviada para reconocimiento")),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Reconocimiento Facial")),
      body: Stack(
        children: [
